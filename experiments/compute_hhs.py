"""
Compute Head Height Stability (HHS) from Kargar et al. (2021).

HHS = sum of |pz(t+1) - pz(t)| for all timesteps

Lower = more stable height. Unit is meters.
...
def compute_hhs(robot_name, coupling, params_path, simulation_time=30.0):
    """
    Re-simulate a robot and compute HHS metric.

    Returns: (hhs, distance, dragging)
    """
...
if __name__ == "__main__":
    ...
    hhs, dist, drag = compute_hhs(robot, coupling, params_path)
    with open(output_file, "w") as f:
        f.write("%s\t%s\t%s\t%s\t%.6f\t%.4f\t%.4f\n" % (
            robot, coupling_label, lam, run_num, hhs, dist, drag
        ))
    print("HHS=%.4f Distance=%.2f Dragging=%.1f%%" % (hhs, dist, drag * 100))
