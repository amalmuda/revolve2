"""Re-simulate two spider controllers for 60s and measure distance/path/EM/drag."""
import os, sys, math, mujoco
import numpy as np
...
def run(robot_name, coupling, evolve_phi0, params_path, sim_time):
    ...
    return disp, path, em, drag_pct


cases = [
    ("spider", "blf", False,
     os.path.expanduser("~/fox_results/spider/spider_bonardi_base_blf_lambda0_nu0.5_w1.0/best_params_run_5.npy"),
     "spider base/blf seed 5"),
    ("spider", "uncoupled", True,
     os.path.expanduser("~/fox_results/spider/spider_bonardi_phi_uncoupled_lambda0_nu0.5_w1.0/best_params_run_3.npy"),
     "spider phi/uncoupled seed 3"),
]

print(f"{'controller':35} {'sim_time':>8} {'dist(m)':>8} {'path(m)':>8} {'EM':>6} {'drag(%)':>8}")
print("-" * 80)
for r, c, p, fp, label in cases:
    for t in (30, 60):
        disp, path, em, drag = run(r, c, p, fp, t)
        print(f"{label:35} {t:>8.0f} {disp:>8.2f} {path:>8.2f} {em:>6.3f} {drag:>8.1f}")
