"""
Evolution for coupled sine brain with offset and coupling.

Parameters per hinge: amplitude, phase, offset (3 per hinge)
Plus global coupling strength.
...
def simulate_coupled_sine_robot(
    robot: ModularRobot,
    simulation_time: float = 10.0,
    terrain_friction: float = 1.0,
) -> tuple[ContactTracker, LocomotionTracker]:
    """Simulate a robot with coupled sine brain and track contacts."""
    # Create terrain
    terrain = Terrain(
        static_geometry=[
            GeometryPlane(
...
def evaluate_coupled_sine_params(
    params: np.ndarray,
    robot_name: str = "spider",
    simulation_time: float = 10.0,
) -> EvalResult:
    """
    Evaluate coupled sine parameters.

    Params format: [amp_0, phase_0, offset_0, ..., coupling_strength, frequency]
    Total: 3*n_hinges + 2
    """
    try:
        ...
        # Extract coupling strength and frequency (last 2 params)
        frequency = float(params[-1])
        coupling_strength = float(params[-2])
        sine_params = params[:-2]
        ...
        contact_tracker, loco_tracker = simulate_coupled_sine_robot(
...
def _eval_wrapper(args):
    ...
    result = evaluate_coupled_sine_params(params, robot_name, sim_time)
    return idx, result


def main():
    """Run 250 generation evolution for coupled sine brain."""
    print("\n" + "=" * 70)
    print("COUPLED SINE BRAIN EVOLUTION - Spider Robot - 250 Generations")
    print("=" * 70)
    ...
    n_params = n_sine_params + 2  # + coupling_strength + frequency

    print(f"\nConfiguration:")
    print(f"  Robot:          {robot_name}")
    print(f"  Hinges:         {n_hinges}")
    print(f"  Parameters:     {n_params} (amp + phase + offset per hinge + coupling + freq)")
    ...
    # Initialize CMA-ES
    # Initial mean: moderate amplitude, zero phase, zero offset, moderate coupling, 1Hz freq
    initial_mean = []
    for _ in range(n_hinges):
        initial_mean.append(0.5)   # amplitude
        initial_mean.append(0.0)   # phase
        initial_mean.append(0.0)   # offset
    initial_mean.append(0.3)  # coupling_strength
    ...
    lower_bounds.append(0.0)   # coupling min
    upper_bounds.append(1.0)   # coupling max
    ...
    print(f"{'Gen':>4} | {'Mean Fit':>9} | {'Max Fit':>9} | {'Best Dist':>10} | "
          f"{'Body Cont':>9} | {'Coupling':>8} | {'Freq':>6} | {'Time':>6}")
    print("-" * 80)
    ...
    for gen in range(num_generations):
        ...
        coupling = solutions[gen_best_idx][-2]
        ...
        history["best_coupling"].append(coupling)
        history["best_freq"].append(freq)

        # Print every generation
        print(f"{gen+1:4d} | {mean_fit:9.4f} | {max_fit:9.4f} | {best_r.distance:10.4f}m | "
              f"{best_r.contact_m1:8.1%} | {coupling:8.3f} | {freq:5.2f} | {gen_time:5.1f}s")

    total_time = time.time() - start_time

    print("-" * 80)
    print("\n" + "=" * 70)
    print("EVOLUTION COMPLETE - COUPLED SINE")
    print("=" * 70)
    ...
    print(f"  Coupling strength:{best_params[-2]:.3f}")
    ...
    print(f"  Coupling: {best_params[-2]:.3f}")
    print(f"  Frequency: {best_params[-1]:.3f} Hz")

    # Save results
    results_file = f"coupled_sine_results_seed{seed}.npz"
    np.savez(
        results_file,
        ...
        history_best_coupling=history["best_coupling"],
    ...
    print("])")
    print(f"coupling_strength = {best_params[-2]:.3f}")
    ...
    if response == 'y':
        ...
        brain = BrainCoupledSine.from_parameters(
            body,
            best_params[:-2],  # sine params (without coupling and freq)
            frequency=best_params[-1],  # evolved frequency
            coupling_strength=best_params[-2],  # evolved coupling
        )
        robot = ModularRobot(body=body, brain=brain)
