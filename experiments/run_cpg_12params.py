"""
Evolution for Revolve2 CPG with 12 parameters (8 internal + 4 external).

Standard CPG with full coupling between hip-knee pairs.
"""
...
def evaluate_cpg_params(
    ...
    """
    Evaluate CPG parameters (12 params for spider).

    Params format: [internal_0, ..., internal_7, external_0, ..., external_3]
    """
    try:
...
def main():
    print("\n" + "=" * 70)
    print("CPG 12 PARAMS (8 internal + 4 external) - 250 Generations")
    print("=" * 70)
    ...
    n_params = cpg_network_structure.num_connections  # 8 internal + 4 external = 12

    print(f"\nConfiguration:")
    print(f"  Robot:          {robot_name}")
    print(f"  Hinges:         {n_hinges}")
    print(f"  Parameters:     {n_params} (8 internal + 4 external)")
    print(f"  Sim time:       {simulation_time}s")
    print(f"  Workers:        {num_workers}")
    print(f"  Seed:           {seed}")
    print(f"  Sigma:          0.5")

    # Initial mean - all weights start at 0.5
    initial_mean = [0.5] * n_params

    # Bounds - weights typically in [-1, 1] or similar
    ...
    print("-" * 70)
    print("\n" + "=" * 70)
    print("EVOLUTION COMPLETE - CPG 12 PARAMS")
    print("=" * 70)
    ...
    print(f"\nBest parameters:")
    print(f"  Internal weights (8): {best_params[:8]}")
    print(f"  External weights (4): {best_params[8:]}")

    results_file = f"cpg_12params_seed{seed}.npz"
    np.savez(
        results_file,
