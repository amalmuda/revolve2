"""
Evolution for Revolve2 CPG with 8 parameters (internal only, external=0).

CPG without coupling - external weights fixed at 0.
"""
...
def evaluate_cpg_params(
    internal_params: np.ndarray,
    robot_name: str = "spider",
    simulation_time: float = 10.0,
) -> EvalResult:
    """
    Evaluate CPG parameters (8 internal only, external=0).

    Params format: [internal_0, ..., internal_7]
    External weights are fixed at 0.
    """
    try:
        ...
        # Combine internal params with zero external weights
        n_internal = cpg_network_structure.num_cpgs  # 8
        n_external = len(cpg_network_structure.connections)  # 4

        # Full params: 8 internal + 4 zeros for external
        full_params = np.concatenate([
            internal_params,
            np.zeros(n_external)  # External weights = 0 (no coupling)
        ])
...
def _eval_wrapper(args):
    ...
    result = evaluate_cpg_params(params, robot_name, sim_time)
    return idx, result


def main():
    print("\n" + "=" * 70)
    print("CPG 8 PARAMS (internal only, external=0) - 250 Generations")
    print("=" * 70)
    ...
    n_params = n_hinges  # Only internal weights = 8

    print(f"\nConfiguration:")
    print(f"  Robot:          {robot_name}")
    print(f"  Hinges:         {n_hinges}")
    print(f"  Parameters:     {n_params} (8 internal, external=0)")
    print(f"  Coupling:       DISABLED (external weights = 0)")
    print(f"  Sim time:       {simulation_time}s")
    print(f"  Workers:        {num_workers}")
    print(f"  Seed:           {seed}")
    print(f"  Sigma:          0.5")

    # Initial mean - internal weights start at 0.5
    initial_mean = [0.5] * n_params

    # Bounds
    lower_bounds = [-2.0] * n_params
    upper_bounds = [2.0] * n_params
    ...
    print("-" * 70)
    print("\n" + "=" * 70)
    print("EVOLUTION COMPLETE - CPG 8 PARAMS (NO COUPLING)")
    print("=" * 70)
    ...
    print(f"\nBest parameters:")
    print(f"  Internal weights (8): {best_params}")
    print(f"  External weights (4): [0, 0, 0, 0] (FIXED)")

    results_file = f"cpg_8params_seed{seed}.npz"
    np.savez(
        results_file,
