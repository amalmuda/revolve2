"""Post-hoc drag/EM measurement for the evolve_nu experiment (on fox).

Re-simulates each evolved controller on its training terrain (in-distribution),
recording distance, em, drag_pct.

Sources:
  ~/revolve2/experiments/results/{robot}_bonardi_evolvenu_{terrain}_nu0.5_w1/
    {robot}_bonardi_phi_nu_uncoupled_lambda0_nu0.5_w1.0/best_params_run_*.npy
    {robot}_bonardi_nu_blf_lambda0_nu0.5_w1.0/best_params_run_*.npy

Output: ~/em_evolvenu.csv
"""
...
def get_struct(robot, coupling, evolve_phi0, evolve_nu):
    ...
    return body, bonardi_structure_from_cpg_structure(
        cpg, evolve_phi0=evolve_phi0, evolve_nu=evolve_nu,
    ), mp
...
def simulate(params, robot, coupling, evolve_phi0, evolve_nu, eval_terrain):
    body, ks, mapping = get_struct(robot, coupling, evolve_phi0, evolve_nu)
...
# Match paths like
# .../robot_bonardi_evolvenu_TERRAIN_nu0.5_w1/robot_bonardi_phi_nu_uncoupled_lambda0_nu0.5_w1.0/best_params_run_N.npy
# .../robot_bonardi_evolvenu_TERRAIN_nu0.5_w1/robot_bonardi_nu_blf_lambda0_nu0.5_w1.0/best_params_run_N.npy
PAT = re.compile(
    r".*/(?P<robot>[^/]+)_bonardi_evolvenu_(?P<terrain>flat|rugged)_nu0\.5_w1/"
    r"(?P=robot)_bonardi_(?P<vc>(?:phi_nu_uncoupled)|(?:nu_blf))_lambda0_nu0\.5_w1\.0/"
...
def task(npy_path):
    ...
    if vc == "phi_nu_uncoupled":
        variant, coupling, evolve_phi0 = "phi", "uncoupled", True
    else:
        variant, coupling, evolve_phi0 = "base", "blf", False
    try:
        params = np.load(npy_path)
        r = simulate(params, robot, coupling, evolve_phi0, True, terrain)
        if r is None:
            return None
        disp, em, drag_pct = r
        return (robot, variant, coupling, terrain, seed, disp, em, drag_pct)
...
if __name__ == "__main__":
    ...
    for r in ROBOTS:
        for terrain in ["flat", "rugged"]:
            outer = os.path.join(BASE, f"{r}_bonardi_evolvenu_{terrain}_nu0.5_w1")
            for sub in ("phi_nu_uncoupled", "nu_blf"):
                pattern = os.path.join(outer, f"{r}_bonardi_{sub}_lambda0_nu0.5_w1.0", "best_params_run_*.npy")
                jobs.extend(sorted(glob.glob(pattern)))
    ...
    out = os.path.expanduser("~/em_evolvenu.csv")
    with open(out, "w") as f:
        f.write("robot,variant,coupling,terrain,seed,distance,em,drag_pct\n")
