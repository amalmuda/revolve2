"""View evolved polar Hopf with custom alpha."""
import math
import sys
import numpy as np
...
PARAMS = sys.argv[2] if len(sys.argv) > 2 else "polar_spider_blf_xy_lam0_1p00hz_alpha1_best.npy"
HZ = float(sys.argv[3]) if len(sys.argv) > 3 else 1.0
SIM_TIME = float(sys.argv[4]) if len(sys.argv) > 4 else 20.0
COUPLING = sys.argv[5] if len(sys.argv) > 5 else "blf"
ALPHA = float(sys.argv[6]) if len(sys.argv) > 6 else 1.0
...
brain = BrainHopfPolarStatic.from_params(
    ...
    omega=2 * math.pi * HZ, alpha=ALPHA,
)
...
print(f"Opening: {ROBOT.upper()} polar Hopf {COUPLING} | omega={HZ} Hz | alpha={ALPHA}")
