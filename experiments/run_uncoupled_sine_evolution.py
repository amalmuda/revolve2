"""
Evolution for uncoupled sine brain with offset and frequency.
...
For fair comparison with coupled sine (26 params = 25 + coupling).
"""
...
from brain_sine_offset import BrainSineOffset, get_num_sine_offset_params
...
def simulate_uncoupled_sine_robot(
    robot: ModularRobot,
    simulation_time: float = 10.0,
    terrain_friction: float = 1.0,
) -> tuple[ContactTracker, LocomotionTracker]:
    """Simulate a robot with uncoupled sine brain and track contacts."""
    # Create terrain
    terrain = Terrain(
        static_geometry=[
            GeometryPlane(
...
def evaluate_uncoupled_sine_params(
    params: np.ndarray,
    robot_name: str = "spider",
    simulation_time: float = 10.0,
) -> EvalResult:
    """
    Evaluate uncoupled sine parameters.
    ...
    try:
        ...
        brain = BrainSineOffset.from_parameters(
            body,
            sine_params,
            frequency=frequency,
        )
        ...
        contact_tracker, loco_tracker = simulate_uncoupled_sine_robot(
...
def _eval_wrapper(args):
    ...
    result = evaluate_uncoupled_sine_params(params, robot_name, sim_time)
    return idx, result


def main():
    """Run 250 generation evolution for uncoupled sine brain."""
    print("\n" + "=" * 70)
    print("UNCOUPLED SINE BRAIN EVOLUTION - Spider Robot - 250 Generations")
    print("=" * 70)
    ...
    body = modular_robots_v1.get(robot_name)
    n_hinges = len(body.find_modules_of_type(ActiveHinge))
    n_sine_params = get_num_sine_offset_params(body)  # 3 * n_hinges
    ...
    print("-" * 80)
    print("\n" + "=" * 70)
    print("EVOLUTION COMPLETE - UNCOUPLED SINE")
    print("=" * 70)
    ...
    # Save results
    results_file = f"uncoupled_sine_results_seed{seed}.npz"
    np.savez(
        results_file,
