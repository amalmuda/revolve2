"""
Evolution for gecko with uncoupled sine brain, FIXED frequency=0.5Hz.

Only evolves: amplitude, phase, offset per hinge (18 params for gecko with 6 hinges).
"""
...
def evaluate_params(
    ...
    """
    Evaluate sine parameters with FIXED frequency.

    Params format: [amp_0, phase_0, offset_0, ...]
    Total: 3*n_hinges (18 for gecko)
    """
    try:
        ...
        for i in range(n_hinges):
            sine_params.append(SineOffsetParameters(
                amplitude=params[3*i],
                frequency=FREQUENCY,
                phase_offset=params[3*i + 1],
                position_offset=params[3*i + 2],
            ))
...
def _eval_wrapper(args):
    idx, params, sim_time = args
    result = evaluate_params(params, sim_time)
    return idx, result


def main():
    print("\n" + "=" * 70)
    print(f"GECKO SINE UNCOUPLED, FREQ={FREQUENCY}Hz - 250 Generations")
    print("=" * 70)
    ...
    n_params = 3 * n_hinges  # amp + phase + offset per hinge

    print(f"\nConfiguration:")
    print(f"  Robot:          gecko (v2)")
    print(f"  Hinges:         {n_hinges}")
    print(f"  Parameters:     {n_params} (amp + phase + offset per hinge)")
    print(f"  Coupling:       NONE (uncoupled)")
    ...
    print("-" * 70)
    print("\n" + "=" * 70)
    print(f"EVOLUTION COMPLETE - GECKO SINE UNCOUPLED, FREQ={FREQUENCY}Hz")
    print("=" * 70)
    ...
    results_file = f"gecko_sine_uncoupled_freq{FREQUENCY}_seed{seed}.npz"
    np.savez(
        results_file,
