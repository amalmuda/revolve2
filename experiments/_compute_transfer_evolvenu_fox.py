"""Cross-terrain transfer test for evolve_nu controllers (on fox).

For each evolve_nu controller, simulate it on the OPPOSITE terrain:
  flat-trained  -> rugged eval
  rugged-trained -> flat eval
...
Output: ~/em_transfer_evolvenu.csv
Columns: robot, variant, coupling, train_terrain, eval_terrain, seed, distance, em, drag_pct
"""
...
def task(args):
    npy_path, eval_terrain = args
    m = PAT.match(npy_path)
    if not m:
        return None
    robot = m.group("robot")
    train_terrain = m.group("terrain")
    vc = m.group("vc")
    ...
    try:
        ...
        r = simulate(params, robot, coupling, evolve_phi0, True, eval_terrain)
        if r is None:
            return None
        disp, em, drag_pct = r
...
if __name__ == "__main__":
        ...
        for terrain in ["flat", "rugged"]:
            ...
            for sub in ("phi_nu_uncoupled", "nu_blf"):
                ...
                for npy_path in sorted(glob.glob(pattern)):
                    eval_terrain = "rugged" if terrain == "flat" else "flat"
                    jobs.append((npy_path, eval_terrain))
    ...
    out = os.path.expanduser("~/em_transfer_evolvenu.csv")
