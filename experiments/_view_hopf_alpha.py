"""View an evolved Hopf robot with arbitrary alpha (convergence rate)."""
import math
import sys
import numpy as np
...
SIM_TIME = float(sys.argv[6]) if len(sys.argv) > 6 else 20.0
...
brain = BrainHopfStatic.from_params(
    ...
    output_mapping=mp, omega=2 * math.pi * HZ, alpha=ALPHA,
)
...
print(f"Opening: {ROBOT.upper()} Hopf {COUPLING} | omega={HZ} Hz | alpha={ALPHA}")
