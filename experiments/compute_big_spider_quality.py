"""
Compute balance, HHS, and EM in a SINGLE simulation pass per big_spider run.
Iterates over uncoupled, neighbour, structured couplings and all 4 lambdas.

Output: _big_spider_quality_metrics.txt with columns:
...
BIG_LOCAL = os.path.expanduser(
    "~/masteroppgave/revolve2/experiments/results/big_robots_local"
)
OUTPUT = os.path.expanduser(
    "~/masteroppgave/revolve2/experiments/_big_spider_quality_metrics.txt"
)
...
def compute_all(robot_name, coupling, params_path, simulation_time=30.0):
    body = modular_robots_v1.get(robot_name)
    active_hinges = body.find_modules_of_type(ActiveHinge)
    cpg_structure, output_mapping = get_cpg_structure(coupling, body, active_hinges)
...
def main():
    ...
    tasks = []
    for cpl_key, _ in couplings:
        for lam in [0, 1, 2, 3]:
            exp_dir = "big_spider_ode_cpg_%s_lambda%d_dragging" % (cpl_key, lam)
            exp_path = os.path.join(BIG_LOCAL, exp_dir)
            if not os.path.isdir(exp_path):
                continue
            for run_num in range(1, 31):
                npy = os.path.join(exp_path, "best_params_run_%d.npy" % run_num)
                if os.path.exists(npy):
                    tasks.append((cpl_key, lam, run_num, npy))

    total = len(tasks)
    print("Will process %d big_spider runs" % total)

    lines = ["morphology\tcoupling\tlambda\trun\tbalance\thhs\tem\tdistance\tdragging"]
    coupling_label_map = dict(couplings)

    for i, (cpl_key, lam, run_num, npy) in enumerate(tasks, 1):
        try:
            b, h, e, d, dr = compute_all("big_spider", cpl_key, npy)
            cpl_label = coupling_label_map[cpl_key]
            lines.append("big_spider\t%s\t%d\t%d\t%.6f\t%.6f\t%.6f\t%.4f\t%.4f" % (
                cpl_label, lam, run_num, b, h, e, d, dr
            ))
            print("[%d/%d] %s lambda=%d run=%d: B=%.3f HHS=%.3f EM=%.3f dist=%.2f drag=%.1f%%" % (
                i, total, cpl_key, lam, run_num, b, h, e, d, dr * 100
            ))
        except Exception as ex:
            print("[%d/%d] ERROR %s lambda=%d run=%d: %s" % (i, total, cpl_key, lam, run_num, ex))
