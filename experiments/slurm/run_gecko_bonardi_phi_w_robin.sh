#!/bin/bash
#SBATCH --job-name=gecko_bon_phiw
...
RESULTS_DIR="results/gecko_bonardi_phi_w_nu1"
...
LOG_DIR="slurm/logs/gecko_bonardi_phi_w/${EXPERIMENT_NAME}"
mkdir -p "$LOG_DIR"
exec > "${LOG_DIR}/run_${RUN_NUM}.log" 2>&1

echo "GECKO BONARDI+phi+w - $EXPERIMENT_NAME run $RUN_NUM seed $SEED"
