...
# Skip uncoupled (tasks 1-20) because +w does nothing on 0-edge graphs.
...
RESULTS_DIR="results/gecko_bonardi_w_nu1"
...
LOG_DIR="slurm/logs/gecko_bonardi_w/${EXPERIMENT_NAME}"
mkdir -p "$LOG_DIR"
exec > "${LOG_DIR}/run_${RUN_NUM}.log" 2>&1

echo "GECKO BONARDI+w - $EXPERIMENT_NAME run $RUN_NUM seed $SEED"
