...
# ============================================
# COMPUTE EM METRIC (Kargar et al. 2021)
...
OUTPUT_DIR="results/em_results"
mkdir -p "$OUTPUT_DIR"
OUTPUT_FILE="${OUTPUT_DIR}/em_${TASK_ID}.txt"

echo "Task $TASK_ID: $ROBOT $COUPLING_LABEL lambda=$LAMBDA run=$RUN_NUM"

source ~/myenv/bin/activate

python3 compute_em.py "$ROBOT" "$COUPLING" "$PARAMS_PATH" "$OUTPUT_FILE" "$COUPLING_LABEL" "$LAMBDA" "$RUN_NUM"
