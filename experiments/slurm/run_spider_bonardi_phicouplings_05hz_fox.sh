...
# Spider Bonardi 0.5 Hz, +phi0 with structured couplings:
# 2 conditions x 2 lambdas x 5 seeds = 20
# Conditions: 0=phi_neighbor, 1=phi_blf
...
POPULATION=25
WORKERS=25
RESULTS_DIR="results/spider_bonardi_nu0.5_w1"
...
case $COND_IDX in
    0) COUPLING="neighbor"; EXTRA_FLAGS="--evolve-phi0"; VARIANT="phi" ;;
    1) COUPLING="blf";      EXTRA_FLAGS="--evolve-phi0"; VARIANT="phi" ;;
esac

cd ~/revolve2/experiments
EXPERIMENT_NAME="${ROBOT}_bonardi_${VARIANT}_${COUPLING}_lambda${LAMBDA}_nu${NU}_w${W}"
LOG_DIR="slurm/logs/spider_bonardi_phic_05hz/${EXPERIMENT_NAME}"
mkdir -p "$LOG_DIR"
