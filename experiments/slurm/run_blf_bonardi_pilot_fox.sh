...
# Pilot: Bonardi CPG with BLF coupling — flat terrain, 4 fitnesses.
# 6 robots x 4 fitness functions x 5 seeds = 120 tasks.
# Layout: robot 0..5 (20 each); within robot: fitness 0..3 (5 each).
...
LOG_DIR="slurm/logs/bonardi_blf_pilot/${ROBOT}_${FITNESS}"
mkdir -p "$LOG_DIR"
...
python -u evolve_blf_bonardi_pilot.py \
    --robot "$ROBOT" --fitness "$FITNESS" --terrain flat --sim-time "$SIM_TIME" \
