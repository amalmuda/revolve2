#!/bin/bash
#SBATCH --job-name=spider_bon_phi
...
# Bonardi variant: + evolved initial phases phi0
# Extra params: n (8 for spider). Spider counts: uncoupled=24, neighbor=28, blf=34.
...
RESULTS_DIR="results/spider_bonardi_phi_nu1_w1"
...
LOG_DIR="slurm/logs/spider_bonardi_phi/${EXPERIMENT_NAME}"
mkdir -p "$LOG_DIR"
exec > "${LOG_DIR}/run_${RUN_NUM}.log" 2>&1

echo "SPIDER BONARDI+phi0 - $EXPERIMENT_NAME run $RUN_NUM seed $SEED"
