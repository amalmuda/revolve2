...
# Bonardi variant: + evolved per-edge coupling weights w_ij
# Extra params: nc. Spider counts: uncoupled=16, neighbor=24, blf=36.

ROBOT="spider"
CONTROLLER="bonardi_w"
...
RESULTS_DIR="results/spider_bonardi_w_nu1"
...
LOG_DIR="slurm/logs/spider_bonardi_w/${EXPERIMENT_NAME}"
mkdir -p "$LOG_DIR"
exec > "${LOG_DIR}/run_${RUN_NUM}.log" 2>&1

echo "SPIDER BONARDI+w - $EXPERIMENT_NAME run $RUN_NUM seed $SEED"
