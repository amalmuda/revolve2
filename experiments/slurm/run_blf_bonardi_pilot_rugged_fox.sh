...
# Pilot: Bonardi CPG with BLF coupling — rugged terrain, 4 fitnesses.
# 6 robots x 4 fitness functions x 5 seeds = 120 tasks.
...
NFS_RESULTS_DIR="results/bonardi_blf_pilot"

cd ~/revolve2/experiments

LOCAL_TMP="${SLURM_TMPDIR:-/tmp/slurm_${SLURM_JOB_ID}_${TASK_ID}}"
mkdir -p "$LOCAL_TMP"
LOCAL_RESULTS_DIR="$LOCAL_TMP/results/bonardi_blf_pilot"
mkdir -p "$LOCAL_RESULTS_DIR"

LOG_DIR="slurm/logs/bonardi_blf_pilot_rugged/${ROBOT}_${FITNESS}"
mkdir -p "$LOG_DIR"
exec > "${LOG_DIR}/run_${RUN_NUM}.log" 2>&1

echo "${ROBOT} rugged fitness=${FITNESS} run ${RUN_NUM} seed ${SEED}"

source ~/myenv/bin/activate

python -u evolve_blf_bonardi_pilot.py \
    --robot "$ROBOT" --fitness "$FITNESS" --terrain rugged --sim-time "$SIM_TIME" \
