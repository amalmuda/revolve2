...
RESULTS_DIR="results/gecko_bonardi_nu1_w1"
...
EXPERIMENT_NAME="${ROBOT}_${CONTROLLER}_base_${COUPLING}_lambda${LAMBDA}_nu${NU}_w${W}"
LOG_DIR="slurm/logs/gecko_bonardi/${EXPERIMENT_NAME}"
mkdir -p "$LOG_DIR"
exec > "${LOG_DIR}/run_${RUN_NUM}.log" 2>&1

echo "GECKO BONARDI base - $EXPERIMENT_NAME run $RUN_NUM seed $SEED"
