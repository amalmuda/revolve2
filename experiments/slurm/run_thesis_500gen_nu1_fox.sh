...
# Same Experiment 1 design (phi/uncoupled, base/blf; fixed w=1, FIXED nu=1.0)
# at 500 generations and 30 seeds, both terrains, all 6 morphologies.
# Per-gen convergence written by python to convergence_run_N.csv (NFS-safe).
#
# This is a control for the evolve_nu finding: if fixed nu=1.0 already
# matches/beats nu=0.5, then part of evolve_nu's gain comes from accessing
# higher frequencies, not from per-joint variation.
#
...
NFS_RESULTS_DIR="results/${ROBOT}_bonardi_500gen_thesis_nu1.0_${TERRAIN}_w1"

cd ~/revolve2/experiments

LOCAL_TMP="${SLURM_TMPDIR:-/tmp/slurm_${SLURM_JOB_ID}_${TASK_ID}}"
mkdir -p "$LOCAL_TMP"
LOCAL_RESULTS_DIR="$LOCAL_TMP/results/${ROBOT}_bonardi_500gen_thesis_nu1.0_${TERRAIN}_w1"
mkdir -p "$LOCAL_RESULTS_DIR"

LOG_DIR="slurm/logs/thesis_500gen_nu1.0/${ROBOT}_${TERRAIN}_${COND_LABEL}"
mkdir -p "$LOG_DIR"
exec > "${LOG_DIR}/run_${RUN_NUM}.log" 2>&1

echo "${ROBOT} ${TERRAIN} ${COND_LABEL} 500gen nu=1.0 lambda=0 (no-drag) run ${RUN_NUM} seed ${SEED}"
