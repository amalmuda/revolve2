...
# Squared-penalty pilot: revolve2 BLF, F2sq/F3sq/F4sq, [-1,1] bounds, flat terrain.
# 6 robots x 3 fitness x 5 seeds = 90 tasks.

ROBOTS=(spider gecko babya queen insect ege2)
FITNESSES=(f2sq f3sq f4sq)
NUM_SEEDS=5
...
# 6 robots x 15 tasks per robot (3 fits x 5 seeds)
...
NFS_RESULTS_DIR="results/revolve2_blf_pilot_sqpen"

cd ~/revolve2/experiments

LOCAL_TMP="${SLURM_TMPDIR:-/tmp/slurm_${SLURM_JOB_ID}_${TASK_ID}}"
mkdir -p "$LOCAL_TMP"
LOCAL_RESULTS_DIR="$LOCAL_TMP/results/revolve2_blf_pilot_sqpen"
mkdir -p "$LOCAL_RESULTS_DIR"

LOG_DIR="slurm/logs/revolve2_blf_pilot_sqpen/${ROBOT}_${FITNESS}"
mkdir -p "$LOG_DIR"
